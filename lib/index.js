  ...
  return function (name) {
    ...
    class Model {
      ...
      async fetch() {
        ...
        var data = await this.collection.findOne({ _id: this._id })
        extend(this, data)
      ...
      async save() {
        ...
        // Insert the document
        var doc = extend({}, this)
        extend(this, await this.collection.insert(doc))
      ...
      async update(data) {
        ...
        // Update and re-fetch
        var doc = extend({}, this)
        await this.collection.update({ _id: this._id }, doc)
        await this.fetch()
      ...
      static async find(query, options) {
        var docs = await this.collection.find(query, options)
      ...
      static async findOne(query, options) {
        var doc = await this.collection.findOne(query, options)
        if ( ! doc) return
      ...
      static async findById(id, options) {
        var doc = await this.collection.findOne({ _id: id }, options)
        if ( ! doc) return
        return this.build(doc)
      }
